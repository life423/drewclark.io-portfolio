...
export default function ProjectCard({ 
  ...
  const handleQuestionSubmit = async (e) => {
    ...
    try {
      ...
      const response = await answerProjectQuestion(projectData, userQuestion);
    ...
    <div className="my-8 overflow-hidden rounded-xl shadow-lg bg-brandGray-800 border border-brandGray-700 transform transition-all duration-300 hover:shadow-xl hover:border-brandGray-600 shadow-[0_0_20px_-5px_rgba(16,185,129,0.15)]">
      ...
      <div className="p-5">
        <div 
          className={clsx(
            ...
            expanded ? "max-h-[800px]" : "max-h-[120px] overflow-hidden relative"
          )}
        >
          {!expanded && (
            <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-brandGray-800 to-transparent"></div>
          )}
          <p>{initialStoryText}</p>
          
          {expanded && (
            <>
              <h3>The Challenge</h3>
              <p>This project presented numerous technical and design challenges that pushed our team to innovate.</p>
              
              <h3>Key Innovations</h3>
              <ul>
                <li>Implemented real-time data processing pipelines</li>
                <li>Designed an intuitive interface for complex information</li>
      ...
      <div className="border-t border-brandGray-700 bg-brandGray-850 py-3">
        <div className="flex justify-center">
          <span className="text-xs text-brandGray-500">Chapter {chapterNumber}</span>
        </div>
      </div>
    </div>
  );
