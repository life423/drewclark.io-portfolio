...
export default function Connect4Game() {
    // Animation state for dropping pieces
    ...
    return (
        ...
        >
            ...
            <Connect4Board
                board={gameState.board}
                onColumnClick={col => {
                    // Only allow clicks if it's player's turn, the game is in progress, and no piece is currently dropping
                    if (
                        gameState.currentPlayer === PLAYER &&
                        gameState.gameStatus === 'playing' &&
                        !droppingPiece
                    ) {
                        // Calculate target row (lowest empty row in column)
                        const column = gameState.board.map(row => row[col])
                        const targetRow = column.lastIndexOf(null)

                        // Ignore if column is full
                        if (targetRow === -1) return

                        // Set the dropping piece animation
                        setDroppingPiece({
                            column: col,
                            row: targetRow,
                            player: PLAYER,
                        ...
                        }, 550) // Slightly longer than the animation to ensure it completes
                    }
                }}
