...
async function checkDockerRunning() {
    try {
        ...
        if (error.message.includes('not found')) {
            log(
                'Docker is not installed. Please install Docker to use container features.',
                'error'
            )
        } else {
            log(
...
async function startQdrantContainer() {
    try {
        ...
        if (!fs.existsSync(config.qdrantComposeFile)) {
            log(
                `Docker compose file not found: ${config.qdrantComposeFile}`,
                'error'
            )
            return false
        }
...
async function waitForQdrantHealth() {
        ...
        for (const endpoint of healthEndpoints) {
            try {
                ...
                if (response.statusCode >= 200 && response.statusCode < 300) {
                    log(
                        `Qdrant is responding on endpoint ${endpoint}`,
                        'success'
                    )
                    isHealthy = true
                    break
                ...
                if (endpoint === healthEndpoints[healthEndpoints.length - 1]) {
                    log(
                        `Health check error (endpoint ${endpoint}): ${error.message}`,
                        'info'
                    )
                }
            }
    ...
    if (!isHealthy) {
        log(
            'Qdrant health check timed out. Vector search may not work properly.',
            'warning'
        )
        return false
    } else {
...
async function ensureCollections() {
    try {
        ...
        // Log the attempt
        log('Retrieving collections from Qdrant...', 'info')
        ...
        try {
            ...
            // Debug the response
            log('Response status code: ' + response.statusCode, 'debug')

            // Handle the response - Qdrant returns collections in result.collections
            ...
            if (
                ...
                existingCollections = response.data.result.collections.map(
                    c => c.name
                )
                log(
                    `Found ${existingCollections.length} existing collections`,
                    'success'
                )
            }
            ...
            for (const collection of collections) {
                if (!existingCollections.includes(collection)) {
                    ...
                    if (
                        ...
                        log(
                            `Failed to create collection ${collection}: ${createResponse.statusCode}`,
                            'error'
                        )
                    }
                } else {
                    log(`Collection ${collection} already exists`, 'info')
                }
            }

            log('Collection check completed successfully', 'success')
            return true
        } catch (requestError) {
            log(`HTTP request failed: ${requestError.message}`, 'error')
            console.error('Request error details:', requestError)
            return false
        }
    } catch (error) {
        log(
            `Could not retrieve collections from Qdrant: ${error.message}`,
            'error'
        )
        return false
    }
...
async function main() {
    ...
    try {
        ...
        if (!dockerRunning) {
            log(
                'Continuing without Docker. Vector search will use mock implementation.',
                'warning'
            )
        ...
        if (!qdrantStarted) {
            log(
                'Failed to start Qdrant container. Vector search will use mock implementation.',
                'error'
            )
        ...
        if (!qdrantHealthy) {
            log(
                'Qdrant is not responding to health checks. Vector search may be unreliable.',
                'warning'
            )
        ...
        const collectionsEnsured = await withRetry(() => ensureCollections(), {
            retries: 5,
            initialDelay: 2000,
            onRetry: (err, attempt) =>
                log(
                    `Retry collection creation (${attempt}/5): ${err.message}`,
                    'warning'
                ),
        })
        ...
        if (!collectionsEnsured) {
            log(
                'Failed to ensure collections. Vector search may be unreliable.',
                'error'
            )
...
if (require.main === module) {
    main()
        .then(result => {
            if (!result.success) {
                log(
                    `Qdrant setup completed with issues: ${result.reason}`,
                    'warning'
                )
                process.exit(1)
            } else {
