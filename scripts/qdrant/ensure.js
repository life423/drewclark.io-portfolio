...
async function getQdrantStatus() {
  try {
    ...
    const dockerRunning = await checkDockerRunning();
    if (!dockerRunning) {
      return { running: false, reason: 'docker-not-running' };
    }
    
    // Check if Qdrant container is running
    const qdrantRunning = await isQdrantRunning();
    if (!qdrantRunning) {
      return { running: false, reason: 'container-not-running' };
    }
    
    // Check if Qdrant is healthy
    try {
      const url = `http://localhost:${config.qdrantPort}/healthz`;
      const response = await httpRequest(url);
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        return { 
          running: true, 
          healthy: false,
          reason: `Unhealthy response: ${response.statusCode}`
        };
      }
    } catch (error) {
      return { 
 ...
 */
async function stopQdrantContainer() {
  try {
    log('Stopping Qdrant container...', 'docker');
    
    // Check if Qdrant is running
    if (!(await isQdrantRunning())) {
      log('Qdrant container is not running', 'info');
      return true;
    }
    
    // Stop the container
    await execPromise('docker rm -f qdrant');
    
    log('Qdrant container stopped and removed', 'success');
    return true;
  } catch (error) {
    log(`Error stopping Qdrant: ${error.message}`, 'error');
    return false;
  }
}
